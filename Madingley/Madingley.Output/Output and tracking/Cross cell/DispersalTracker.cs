...
namespace Madingley
{
    ...
    public class DispersalTracker
    {
        ...
        {
            this.DispersalWriter.Close();
            this.SyncedDispersalWriter.Close();

            System.IO.File.Copy(existing.FileName, this.FileName, true);

            this.DispersalWriter = new StreamWriter(this.FileName, true);
            this.SyncedDispersalWriter = TextWriter.Synchronized(this.DispersalWriter);
        }
