    ...
    public class GridCell
    {
            ...
            for (int FunctionalGroup = 0; FunctionalGroup < functionalGroups.GetNumberOfFunctionalGroups(); FunctionalGroup++)
            {
                ...
                if (FunctionalGroupsToUse.Contains(FunctionalGroup))
                {
                    if (_CellEnvironment["Realm"][0] == 1.0)
                    {
                        ...
                        if (noPlantModels == null || noPlantModels.Contains(FunctionalGroup) == false)
                        {
                            LeafMass = PlantModel.CalculateEquilibriumLeafMass(_CellEnvironment, functionalGroups.GetTraitNames("leaf strategy", FunctionalGroup) == "deciduous");
                        }
#else
        ...
        public class CohortComparer : IComparer<Cohort>
        {
            public int Compare(Cohort x, Cohort y)
            {
                ...
                int retval = x.CohortID.Count.CompareTo(y.CohortID.Count);
                if (retval != 0) return retval;

                for (var i = 0; i < x.CohortID.Count; ++i)
                {
                    retval = x.CohortID[i].CompareTo(y.CohortID[i]);
                    if (retval != 0) return retval;
                }
        ...
        public class StockComparer : IComparer<Stock>
        {
            public int Compare(Stock x, Stock y)
            {
                ...
                int retval = x.FunctionalGroupIndex.CompareTo(y.FunctionalGroupIndex);
                if (retval != 0) return retval;

                retval = x.IndividualBodyMass.CompareTo(y.IndividualBodyMass);
                if (retval != 0) return retval;

                return x.TotalBiomass.CompareTo(y.TotalBiomass);
            }
        }
